...
public class Handler extends TextWebSocketHandler
{
  ...
  private Process streamProc = null;
  @Value("${rtr.tool-folder}")
  private String toolFolder;

  private Boolean logIce = false;
  ...
  {
    if(logIce) log.info("##################  afterConnectionClosed   #################");
  ...
  {
    if(logIce) log.info("##################  handleTextMessage: " + message);
  ...
  //=============================================================================
  private void handleHeartbeat(TextMessage message, WebSocketSession session) {
    ...
    if(user != null) {
      ...
      log.info("  ---- RTP EP stats: ----");
      MediaType mediaType = MediaType.VIDEO;
      Map<String, Stats> statsMap = rtpEp.getStats(mediaType);
      for (Map.Entry<String,Stats> entry : statsMap.entrySet()) {
        //log.info("Key = " + entry.getKey() +
        //        ", Type = " + entry.getValue().getType().name());
        if(entry.getValue().getType() == StatsType.inboundrtp) {
          RTCInboundRTPStreamStats st = (RTCInboundRTPStreamStats) entry.getValue();
          log.info("       SSRC:          " + st.getSsrc());
          log.info("       PacketsRXed:   " + st.getPacketsReceived());
          log.info("       BytesRXed:     " + st.getBytesReceived());
        }
      }
      log.info("  ---- WEB EP stats: ----");
      mediaType = MediaType.VIDEO;
      statsMap = webEp.getStats(mediaType);
      for (Map.Entry<String,Stats> entry : statsMap.entrySet()) {
        //log.info("Key = " + entry.getKey() +
        ...
        if(entry.getValue().getType() == StatsType.outboundrtp) {
            RTCOutboundRTPStreamStats st = (RTCOutboundRTPStreamStats) entry.getValue();
            log.info("       PacketsSent:   " + st.getPacketsSent());
            log.info("       BytesSent:     " + st.getBytesSent());
            log.info("       Status:        " + webEp.getConnectionState().toString());
        }
      }
    }
    else {
                           ...
                           JsonObject jsonMessage)
  {
    ...
    if(logIce) log.info("############# {handleError} ...");
    stop(session);
  }
  ...
  public void handleTransportError(WebSocketSession session, Throwable ex)
      throws Exception
  {
    if(logIce) log.info("##################  handleTransportError   #################");
  ...
  {
    if(logIce) log.info("##################  addBaseEventListeners   #################");
  ...
  {
    if(logIce) log.info("##################  addWebRtcEventListeners   #################");
  ...
  {
    if(logIce) log.info("##################  initWebRtcEndpoint  #################");
...
===============================================================================================*/
    ...
    webRtcEp.setTurnUrl("dqft:Digiteq2018@192.168.1.54:11000");    // @todo change security type
  ...
  {
    if(logIce) log.info("##################  startWebRtcEndpoint   #################");
  ...
  {
    if(logIce) log.info("##################  makeRtpEndpoint   #################");
    if (!useSrtp) {
  ...
  */
  ...
  {
    if(logIce) log.info("##################  startRtpEndpoint   #################");
...
*/
    ...
    this.setRtpStreamProvider(senderIp, kmsRtpPortV, kmsIp);
      ...
      JsonObject jsonMessage)
  {
    ...
    if(logIce) log.info("##################  handleProcessSdpOffer   #################");
    String sessionId = session.getId();
  ...
  private void handleAddIceCandidate(final WebSocketSession session,
      JsonObject jsonMessage)
  {
    if(logIce) log.info("##################  handleAddIceCandidate  #################");
    String sessionId = session.getId();
    ...
    if (user != null) {
      ...
      String scandidate = jsonCandidate.get("candidate").getAsString();
      if(scandidate.length() < 5) {
          log.info("+++++++ Empty candidate !!!!!");
          return;
      }
  ...
  {
    if(logIce) log.info("##################  sendPlayEnd   #################");
  ...
  {
    if(logIce) log.info("##################  sendError:  " + errMsg);
    if (users.containsKey(session.getId())) {
      JsonObject message = new JsonObject();
  ...
  private synchronized void sendMessage(final WebSocketSession session,
      String message)
  {
    if(logIce) log.info("##################  sendMessage: " + message);
    if (!session.isOpen()) {
  ...
  private int killGst() {
    log.info("************  Killing GST ..... ");
    Runtime rt = Runtime.getRuntime();

    try {
      if (streamProc != null) {
        streamProc.destroy();
        streamProc = rt.exec(toolFolder + "kill-gst.sh");
        log.info("Killing gst-launch processes .... ");
        streamProc.waitFor();
      }
    }
    catch(Exception ee) {
  ...
  private void setRtpStreamProvider(String senderIp, int kmsRtpPortV, String kmsIp) {
    String scmd = toolFolder + "start-stream-movie.sh " + kmsRtpPortV + " " + kmsIp;

    log.info("************  Starting: " + scmd);
    Runtime rt = Runtime.getRuntime();

    killGst();

    try {
      streamProc = rt.exec(scmd);
      log.info(String.format("New process: '%s' ", scmd));
    } catch (Exception e) {
      e.printStackTrace();
    }
